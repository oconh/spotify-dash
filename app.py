...
def page_audio_dna(time_range):
    tracks = get_top_tracks(sp, time_range, limit=50)
    features = get_audio_features(sp, [t["id"] for t in tracks])
    df_feat = pd.DataFrame(features)

    radar_cols = ["energy", "valence", "danceability",
                  "acousticness", "instrumentalness", "speechiness"]
    means = df_feat[radar_cols].mean()
    ...
    df_merged = df_tracks.merge(df_feat[["id", "valence", "energy"]], on="id", how="inner")

    fig_mood = go.Figure(go.Scatter(
        x=df_merged["valence"],
        y=df_merged["energy"],
        mode="markers+text",
        text=df_merged["name"].str[:20],
        textposition="top center",
        textfont=dict(size=9, color=COLORS["muted"]),
        marker=dict(
            size=10,
            color=df_merged["valence"],
            colorscale=[[0, COLORS["accent2"]], [0.5, COLORS["accent"]], [1, "#fee440"]],
            opacity=0.8,
            showscale=False,
        ),
    ...
    fig_mood.add_shape(type="line", x0=0.5, x1=0.5, y0=0, y1=1,
                       line=dict(color=COLORS["border"], dash="dash"))
    fig_mood.add_shape(type="line", x0=0, x1=1, y0=0.5, y1=0.5,
                       line=dict(color=COLORS["border"], dash="dash"))
    for x, y, label in [(0.25, 0.85, "Angry"), (0.75, 0.85, "Happy"),
                          (0.25, 0.15, "Sad"),   (0.75, 0.15, "Peaceful")]:
        fig_mood.add_annotation(x=x, y=y, text=label, showarrow=False,
                                 font=dict(size=11, color=COLORS["muted"]))
    fig_mood.update_layout(
        **PLOTLY_LAYOUT,
        title="Mood Map",
        xaxis_title="Valence (sadder → happier)",
        yaxis_title="Energy (calmer → more energetic)",
        height=420,
    )
    ...
    return html.Div([
        html.H2("Audio DNA", style={"marginBottom": "0.5rem", "fontWeight": "700"}),
        html.P(TIME_RANGES[time_range], style={"color": COLORS["muted"], "marginBottom": "1.5rem"}),
        html.Div([
            stat_card("Energy",       pct(means["energy"])),
            stat_card("Valence",      pct(means["valence"]),      COLORS["accent2"]),
            stat_card("Danceability", pct(means["danceability"]), COLORS["accent3"]),
            stat_card("Acousticness", pct(means["acousticness"]), "#00bbf9"),
        ], style={"display": "grid", "gridTemplateColumns": "repeat(4,1fr)",
                  "gap": "1rem", "marginBottom": "1.5rem"}),
        html.Div([
            card([dcc.Graph(figure=fig_radar, config={"displayModeBar": False})]),
            card([dcc.Graph(figure=fig_mood,  config={"displayModeBar": False})]),
